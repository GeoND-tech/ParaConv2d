...
# This is a build script for a PyTorch custom op C++/CUDA shared object file for PyTorch versions earlier than 2.13.0. For versions 2.13.0+ use buildpt.sh instead.
...
nvcc -std=c++17 -v -c -o ./$1kernels.cu.o ./$1kernels.cu -x cu -Xcompiler -fPIC --expt-relaxed-constexpr -arch sm_60 --device-c -DUSE_C10D_GLOO -DUSE_C10D_NCCL -DUSE_DISTRIBUTED -DUSE_RPC -DUSE_TENSORPIPE -isystem $TORCH_INCLUDE -isystem $TORCH_API_INCLUDE -isystem $PYTHON_INCLUDE -D_GLIBCXX_USE_CXX11_ABI=1



# Build the backward pass kernels object file.

nvcc -std=c++17 -v -c -o ./$1gradkernels.cu.o ./$1gradkernels.cu -x cu -Xcompiler -fPIC --expt-relaxed-constexpr -arch sm_60 --device-c -DUSE_C10D_GLOO -DUSE_C10D_NCCL -DUSE_DISTRIBUTED -DUSE_RPC -DUSE_TENSORPIPE -isystem $TORCH_INCLUDE -isystem $TORCH_API_INCLUDE -D_GLIBCXX_USE_CXX11_ABI=1



# Build the CUDA computations object file.

nvcc -std=c++17 -v -c -o $1.cu.o $1.cu -x cu -Xcompiler -fPIC --expt-relaxed-constexpr -arch sm_60 --device-c -DUSE_C10D_GLOO -DUSE_C10D_NCCL -DUSE_DISTRIBUTED -DUSE_RPC -DUSE_TENSORPIPE -isystem $TORCH_INCLUDE -isystem $TORCH_API_INCLUDE -isystem $PYTHON_INCLUDE -D_GLIBCXX_USE_CXX11_ABI=1
...
/usr/bin/c++ -std=c++17 -DUSE_C10D_GLOO -DUSE_C10D_NCCL -DUSE_DISTRIBUTED -DUSE_RPC -DUSE_TENSORPIPE -Dptparaboloid_EXPORTS -isystem $TORCH_INCLUDE -isystem $TORCH_API_INCLUDE -fPIC -isystem $PYTHON_INCLUDE -D_GLIBCXX_USE_CXX11_ABI=1 -o $1.cc.o -c $1.cc
